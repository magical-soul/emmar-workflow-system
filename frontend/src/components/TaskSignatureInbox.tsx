...
export function TaskSignatureInbox({ requests, onResolve }: TaskSignatureInboxProps) {
  return (
      ...
      <div className="flex flex-col gap-3 max-h-75 overflow-y-auto pr-1">
        {requests?.length === 0 ? (
          <div className="text-center py-6 text-xs text-slate-500 italic font-medium">
            Your task inbox is completely clear. No outstanding signature verifications required.
          </div>
        ) : (
          requests?.map((req) => (
            <div key={req.id} className="bg-slate-900/60 border border-slate-700/50 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition duration-200 hover:border-slate-600">
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-[11px] font-medium text-slate-500">
                  Ticket Token ID: <span className="font-mono">...{req.id.slice(-8)}</span>
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 self-end sm:self-center">
                <button
                  ...
                  className="bg-red-500/10 hover:bg-red-500 text-red-400 hover:text-white border border-red-500/20 text-xs font-bold px-3 py-1.5 rounded-lg transition duration-200"
                >
                  Reject
                </button>
                <button
                  onClick={() => onResolve(req.id, 'APPROVED')}
                  className="bg-emerald-500/10 hover:bg-emerald-500 text-emerald-400 hover:text-slate-900 border border-emerald-500/20 text-xs font-bold px-3 py-1.5 rounded-lg transition duration-200 shadow-sm"
                >
                  Approve Auth
                </button>
              </div>
