...
async function runE2ETestSuite() {
  ...
  const tenant = await prisma.tenant.findFirst({
    where: { name: "Emaar Properties" },
  });
  const item = await prisma.item.findFirst({
    where: { title: { contains: "Burj Khalifa" } },
  });

  if (!tenant || !item) {
    console.error(
      'Test Setup Failure: Run "npm run db:seed" before launching the E2E suite.',
    );
    process.exit(1);
  }
  ...
  console.log(
    " Test Case 1: Triggering Automated Transition (DRAFT -> PENDING_APPROVAL)...",
  );
  try {
    ...
    if (res.ok) {
      console.log(
        `✅ Success! New Item State: ${data.data.newState} (Version: ${data.data.itemId ? 2 : "Updated"})`,
      );
    } else {
  ...
  try {
    ...
    // Checks if the call returned a bad status or explicitly contains our error warning message!
    if (
      data.message &&
      data.message.includes("requires an authorized manager signature")
    ) {
      console.log(
        `✅ Success! System Engine successfully blocked illegal transition. Error Message Stamped:\n   "${data.message}"`,
      );
    } else {
      console.log(
        `❌ Fail: System allowed a banned action or returned incorrect state frames.`,
      );
    }
  } catch (err: any) {
    console.error(`Request Exception: ${err.message}`);
